...
permission. These materials have not been saved to the Devpost draft, and the
contest entry has not been submitted.
...
| [Existing Devpost thumbnail](../../public/devpost-thumbnail.png) | 1200×800 PNG; preserved without redesign |
| [Existing video cover](../../public/og.png) | 1200×630 PNG; preserved without redesign |
...
- The tagline is 105 characters. The existing 1200×800 thumbnail is 51,984 bytes.
...
Devpost recommends a 3:2 project thumbnail, JPG/PNG/GIF, maximum 5MB. The existing
1200×800 thumbnail is retained. These thumbnail requirements are not assumed to
apply to the screenshot gallery; its actual upload limits and any additional
contest fields still need checking in the signed-in form.
[Official submission guide](https://help.devpost.com/article/126-know-your-submission-steps).

## Remaining delivery steps

1. Before submitting, confirm the public app includes source correction `809bf1b`
   and that the repository's source matches the deployed build. The creator has
   authorized publication of the correction and this package.
2. Check the already-public YouTube video while signed out, including audio.
   Embedding is enabled. Confirm permission to publish the included voice, font,
   and sound assets. Do not upload a duplicate video.
3. With approval to update the existing Devpost draft, add the video URL, story,
   and screenshots. Inspect the rendered draft and resolve any form-specific
   fields. Do not create a duplicate entry.
4. Submit only after the user's final confirmation. Verify a submission receipt
   or status; a saved draft alone is not a completed entry.
